...
public class VehicleService2 implements VehicleService{
     ...
     */
    @Autowired
    public VehicleService2(Inventory inventory) {
    ...
    public Vehicle findMostExpensive() {
        return inventory.getVehicles().stream().sorted(Comparator.comparing(Vehicle::getPrice).reversed()).limit(1).toList().get(0);
    }
    ...
    public Vehicle findLeastExpensive() {
        return inventory.getVehicles().stream().sorted(Comparator.comparing(Vehicle::getPrice)).limit(1).toList().get(0);
    }
}
